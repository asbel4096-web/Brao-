...
import { formatPrice, normalizePhone, whatsappLink } from '@/lib/utils';

export function ListingCard({ listing }: { listing: Listing & Record<string, any> }) {
  const image = listing.images?.[0] || '/icons/car-card.svg';
  const contactPhone = listing.sellerPhone || listing.phone || '';
  const telHref = normalizePhone(contactPhone) ? `tel:${normalizePhone(contactPhone)}` : '#';
  const whatsappHref = whatsappLink(listing.whatsapp || contactPhone);

  return (
    <div className="card overflow-hidden transition hover:-translate-y-1 hover:shadow-2xl">
      ...
      <div className="p-5">
        ...
        <div className="mt-4 flex flex-wrap gap-2 text-xs text-slate-500">
          ...
          {listing.mileage ? <span className="rounded-full bg-slate-100 px-3 py-1">{Number(listing.mileage).toLocaleString('ar-LY')} كم</span> : null}
          {(listing.fuel || listing.fuelType) ? <span className="rounded-full bg-slate-100 px-3 py-1">{listing.fuel || listing.fuelType}</span> : null}
        ...
        <div className="mt-5 grid grid-cols-3 gap-2">
          ...
          <a href={telHref} className="btn-secondary gap-1 px-3 py-2 text-xs"><Phone size={14} />اتصال</a>
          <a href={whatsappHref} target="_blank" rel="noreferrer" className="btn-primary gap-1 px-3 py-2 text-xs"><MessageCircle size={14} />واتساب</a>
        </div>
      </div>
    </div>
  );
