      ...
      <div className="flex flex-col gap-6 lg:flex-row">
        <aside className="card w-full p-5 lg:w-[320px] lg:self-start">
          ...
          <div className="mt-5 space-y-4">
            ...
            <div><label className="label">القسم</label><select className="input" value={category} onChange={(e) => setCategory(e.target.value)}><option>كل الأقسام</option>{CATEGORY_OPTIONS.map((item) => <option key={item.key}>{item.label}</option>)}</select></div>
            <div><label className="label">المدينة</label><select className="input" value={city} onChange={(e) => setCity(e.target.value)}><option>كل المدن</option>{LIBYAN_CITIES.map((cityName) => <option key={cityName}>{cityName}</option>)}</select></div>
