  ...
  if (loading) {
    return (
      <section className="container py-10">
        <div className="card p-6 text-center text-slate-500">جارٍ تحميل تفاصيل الإعلان...</div>
      </section>
    );
  }
          ...
          <div className="card overflow-hidden">
            <div className="grid gap-3 p-4 sm:grid-cols-2">
              {(listing.images?.length ? listing.images : [fallbackImage]).map((image, index) => (
                <div key={`${image}-${index}`} className="h-[220px] overflow-hidden rounded-3xl bg-slate-100 sm:h-[260px]">
        ...
        <aside className="space-y-6">
          <div className="card p-6">
            ...
            <div className="mt-6 grid gap-3">
              <a href={`tel:${listing.phone}`} className="btn btn-secondary gap-2"><Phone size={18} />اتصال مباشر</a>
              <a href={`https://wa.me/${listing.whatsapp ?? listing.phone}`} className="btn btn-primary gap-2"><MessageCircle size={18} />واتساب</a>
              <Link href="/messages" className="btn btn-secondary">فتح الدردشة</Link>
            </div>
          </div>
          ...
          <div className="card p-6">
            ...
            <div className="mt-4 text-sm leading-7 text-slate-600">تواصل مباشرة مع صاحب الإعلان عبر الاتصال أو واتساب.</div>
          </div>
        </aside>
      </div>
    </section>
